#!/usr/bin/env python3
import tkinter, Geodesy, math, DbManager
...
def LoadDb():
  ...
  DbFileName : str
  DbFileName = tkinter.filedialog.askopenfilename()
  if type(DbFileName) != str :
    return
...
def ConfirmSelection():
  ...
  TxtStartLat.insert(0, SearchResult[SelectedIndex].Lat)
  TxtStartLon.delete(0,tkinter.END)
  TxtStartLon.insert(0, SearchResult[SelectedIndex].Lon)
  MultipleSel.destroy()
...
def ConfirmSelection2():
  ...
  TxtDestLat.insert(0, SearchResult2[SelectedIndex].Lat)
  TxtDestLon.delete(0,tkinter.END)
  TxtDestLon.insert(0, SearchResult2[SelectedIndex].Lon)
  MultipleSel.destroy()
...
def SearchPoint():
  ...
  if len(SearchResult) < 1:
    ...
    TxtStartLat.delete(0,tkinter.END)
    TxtStartLat.insert(0, SearchResult[0].Lat)
    TxtStartLon.delete(0,tkinter.END)
    TxtStartLon.insert(0, SearchResult[0].Lon)
    return
  else:
    global MultipleSel
    ...
    global CmdSelect
    CmdSelect= tkinter.Button(master=MultipleSel, text="Select", font=DefaultFontTuple,
...
def SearchPoint2():
  ...
  if len(SearchResult2) < 1:
    ...
    TxtDestLat.delete(0,tkinter.END)
    TxtDestLat.insert(0, SearchResult2[0].Lat)
    TxtDestLon.delete(0,tkinter.END)
    TxtDestLon.insert(0, SearchResult2[0].Lon)
  else:
    global MultipleSel
    MultipleSel= tkinter.Toplevel(master=home)
    MultipleSel.grab_set()
...
FrameDbInfo = tkinter.LabelFrame(master=home,text=Dbname.get(), font=DefaultFontTuple)
FrameDbInfo.grid(row=0,column=0,columnspan=2)
LblDbVhfCount = tkinter.Label(master=FrameDbInfo, text="VHF: "+str(VhfCount.get()).ljust(5," "), font=DefaultFontTuple,foreground="green")
LblDbVhfCount.grid(row=0,column=0)
LblDbNdbCount = tkinter.Label(master=FrameDbInfo, text="NDB: "+str(NdbCount.get()).ljust(5," "), font=DefaultFontTuple,foreground="red")
LblDbNdbCount.grid(row=0,column=1)
LblDbAptCount = tkinter.Label(master=FrameDbInfo, text="APT: "+str(AptCount.get()).ljust(5," "), font=DefaultFontTuple,foreground="blue")
LblDbAptCount.grid(row=0,column=2)

CmdLoadDb = tkinter.Button(master=home, text="Load", font=DefaultFontTuple,
                          command=LoadDb, image=LoadDbIcon, compound="l")
...
FrameDest = tkinter.LabelFrame(master=home,text="Destination Point", font=DefaultFontTuple)
FrameDest.grid(row=1,column=3,rowspan=2,columnspan=2, padx=padValue, pady=padValue)
LblDestLat = tkinter.Label(master=FrameDest,text="Latitude", font=DefaultFontTuple)
...
LblDestLon = tkinter.Label(master=FrameDest,text="Longitude", font=DefaultFontTuple)
...
CmdSearch1 = tkinter.Button(master=FrameStart, text="Search", font=DefaultFontTuple,
...
CmdInverse = tkinter.Button(master=FrameRoute, text= "Inv",
                            ...
                            image=InverseIcon, compound="l")
CmdInverse.grid(row=4,column=1)
CmdSearch2 = tkinter.Button(master=FrameDest, text="Search", font=DefaultFontTuple,
                            command=SearchPoint2, image=SearchIcon, compound="l",state="disabled")
